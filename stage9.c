...
// Helper to find or allocate a unique dynamic IP index for simultaneous execution
int allocate_next_ip_index(const char *base_dir, const char *container_name) {
    char state_path[512];
    struct stat st;
    
    // Ensure ipam state directory track tracker lives on the host
    snprintf(state_path, sizeof(state_path), "%s/ipam", base_dir);
    mkdir(state_path, 0755);

    // If this container already has an assigned IP index lease, reuse it
    ...
    // Otherwise, scan from .2 up to .254 to look for an unallocated slot
    for (int current_slot = 2; current_slot < 254; current_slot++) {
        int slot_taken = 0;
        char check_path[512];
        
        // Check if any other live container file owns this IP right now
        ...
        if (!slot_taken) {
            // Save the newly allocated index lease mapping
    ...
    return 2; // Fallback default index slot allocation
}
...
void run_container_engine(ContainerConfig *cfg, const char *base_dir, const char *name) {
    ...
    // Dynamic IP lookup allocation computation
    ...
    // Host Side Network Plumbing (Dynamic Central Bridge Initialization)
    char cmd[512];
    
    // 1. Create central bridge interface switch if it doesn't exist
    ...
    // 2. Create localized unique interface wires for this specific box
    snprintf(cmd, sizeof(cmd), "ip link add veth_h_%s type veth peer name veth_c_%s", name, name);
    system(cmd);
    
    // 3. Bind host side wire edge straight into the br0 switch hub
    ...
    // 4. Send the child side edge wire sliding inside the container space map
    snprintf(cmd, sizeof(cmd), "ip link set veth_c_%s netns %d", name, container_pid);
    system(cmd);

    // 5. Force namespace configuration adapters cleanly from outside via nsenter using dynamic targets
    ...
    // 6. Global firewall forwarding rule definitions
    ...
    // Tear down explicit interface wire lines cleanly on shutdown exit sequences
